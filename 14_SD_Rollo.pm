#######################################################################################################################################################
# $Id: 14_SD_Rollo.pm 32 2019-02-11 12:00:00 v3.3.3-dev_05.12. $
#   
...
#		- SD_Rollo_Set -> lt. https://wiki.fhem.de/wiki/DevelopmentModuleIntro -> unknown argument [Parameter] choose one of [Liste möglicher Optionen]
...
#####################################
sub SD_Rollo_Initialize() {
  my ($hash) = @_;
  $hash->{Match}				= "^P(?:87|88)#.*";
	$hash->{DefFn}				= "SD_Rollo_Define";
  $hash->{UndefFn}			= "SD_Rollo_Undef";
  $hash->{AttrFn}				= "SD_Rollo_Attr";
  $hash->{SetFn}				= "SD_Rollo_Set";
  $hash->{ParseFn}			= "SD_Rollo_Parse";
  ...
  $hash->{FW_summaryFn}	= "SD_Rollo_summaryFn";          # displays html instead of status icon in fhemweb room-view
}
...
sub SD_Rollo_Define() {
  ...
  # Argument            				   0	     1       2      3          4
	return " wrong syntax: define <name> SD_Rollo <Serial> <Typ> <optional IODEV> " if(int(@a) < 4 || int(@a) > 5);
  ...
  my $name = $hash->{NAME};
	...
	$modules{SD_Rollo}{defptr}{$hash->{DEF}} = $hash;
	my $ioname = $modules{SD_Rollo}{defptr}{ioname} if (exists $modules{SD_Rollo}{defptr}{ioname} && not $iodevice);
...
sub SD_Rollo_Attr(@) {
	...
	if ($init_done == 1) {
		...
		Log3 $name, 3, "SD_Rollo: $cmd attr $attrName to $attrValue" if (defined $attrValue);
		Log3 $name, 3, "SD_Rollo: $cmd attr $attrName" if (not defined $attrValue);
	}
...
sub SD_Rollo_Set($$$@) {
	...
	if ($Serial_send ne "" && $MasterMSB ne "" && $MasterLSB ne "" && $KeeLoq_NLF ne "") {
		...
		Log3 $name, 5, "$ioname: SD_Rollo_Set - returnlist finish = $ret" if ($cmd ne "?");
	}
  ...
  return $ret if ( $a[0] eq "?");
	...
	Log3 $name, 4, "$ioname: SD_Rollo_Set - cmd=$cmd cmd2=$cmd2 args cmd2=".scalar @channel_split." addGroups=$addGroups" if ($cmd ne "?" && defined $cmd2);
	...
	if ($cmd ne "?") {
		...
		if ($addGroups ne "") {
			...
			foreach my $found (@channel_from_addGroups){
				if ($found =~ /^$cmd2:/) {
					Log3 $name, 4, "$ioname: SD_Rollo_Set - check, group for setlist -> found $cmd2 in addGroups";
					$found =~ s/$cmd2\://g;
		...
		### multi control ###
		if ( grep( /,/, $cmd2 ) ) {
			Log3 $name, 4, "$ioname: SD_Rollo_Set - check, multiple selection: yes";
			my @multicontrol = split(",", $cmd2);
			foreach my $found_multi (@multicontrol){
				## channel ##
				if ($found_multi =~ /^\d$/) {
					Log3 $name, 5, "$ioname: SD_Rollo_Set - selection $found_multi is channel solo";
				## group ##
				} elsif (grep /$found_multi/, @channel_from_addGroups) {
					Log3 $name, 5, "$ioname: SD_Rollo_Set - selection $found_multi is group solo";
					foreach my $found (@channel_from_addGroups) {
						if ($found =~ /^$found_multi:/) {
							Log3 $name, 4, "$ioname: SD_Rollo_Set - check, group for setlist -> found $found_multi in addGroups";
		...
		### single control ###
			...
			Log3 $name, 4, "$ioname: SD_Rollo_Set - check, multiple selection: no";
			if ($cmd2 !~ /^\d$/) {
				## single group ##
				if ( grep( /$cmd2:/, $addGroups ) ) {
					Log3 $name, 4, "$ioname: SD_Rollo_Set - no numerics";
				}
			## single channel ##
			} else {
				@channels = split /,/, $cmd2;			
		...
		Log3 $name, 4, "$ioname: SD_Rollo_Set - channel via setlist | button=$button buttonbits=$buttonbits channel=$channel";
		### END
		...
		my $device_key_lsb = SD_Rollo_decrypt($keylow, hex($MasterMSB), hex($MasterLSB),$name);	# verified
		$keylow = $DeviceKey | 0x60000000;
		my $device_key_msb = SD_Rollo_decrypt($keylow, hex($MasterMSB), hex($MasterLSB),$name);	# verified

		### KEELOQ
		my $disc = $bit0to7."0000".$jaro_channels{$channel};	# Hopcode													# verified
	
		my $result = (SD_Rollo_bin2dec($disc) << 16) | $counter_send;														# verified
		my $encoded = SD_Rollo_encrypt($result, $device_key_msb, $device_key_lsb,$name);				# verified
		...
		Log3 $name, 5, "$ioname: SD_Rollo_Set - Channel                   = $channel";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - channelpart1 (Group 0-7)  = $bit0to7";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - channelpart2 (Group 8-15) = $bit64to71";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - Button                    = $button";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - Button_bits               = $buttonbits";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - DeviceKey                 = $DeviceKey";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - Device_key_lsb            = $device_key_lsb";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - Device_key_msb            = $device_key_msb";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - disc                      = $disc";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - result (decode)           = $result";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - Counter                   = $counter_send";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - encoded (encrypt)         = ".sprintf("%032b", $encoded)."\n";

		my $binsplit = SD_Rollo_binsplit_JaroLift($bits);

		Log3 $name, 5, "$ioname: SD_Rollo_Set                                                   encoded     <- | ->     decrypts";
		Log3 $name, 5, "$ioname: SD_Rollo_Set                               Grp 0-7 |digitS/N|      counter    | ch |          serial        | bt |Grp 8-15";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - bits (send split)         = $binsplit";
		Log3 $name, 5, "$ioname: SD_Rollo_Set - bits (send)               = $bits";
		Log3 $name, 4, "$ioname: SD_Rollo_Set - sendMSG                   = $msg";
...
###################################
sub SD_Rollo_dec2bin($) {
	my $bin = unpack("B32", pack("N", shift));
	return $bin;
}

###################################
sub SD_Rollo_translate($) {
	my $text = shift;
...
sub SD_Rollo_encrypt($$$$){
	...
	while ($r < 528){
		my $keyBitNo = $r & 63;
		if ($keyBitNo < 32){
			$keyBitVal = SD_Rollo_bitRead($_keyLow, $keyBitNo);
		} else {
			$keyBitVal = SD_Rollo_bitRead($_keyHigh, $keyBitNo - 32);
		}		
		$index = 1 * SD_Rollo_bitRead($x,1) + 2 * SD_Rollo_bitRead($x,9) + 4 * SD_Rollo_bitRead($x,20) + 8 * SD_Rollo_bitRead($x,26) + 16 * SD_Rollo_bitRead($x,31);
		$bitVal = SD_Rollo_bitRead($x,0) ^ SD_Rollo_bitRead($x, 16) ^ SD_Rollo_bitRead($KeeLoq_NLF,$index) ^ $keyBitVal;
...
sub SD_Rollo_decrypt($$$$){
	...
	while ($r < 528){
		...
		$index = 1 * SD_Rollo_bitRead($x,0) + 2 * SD_Rollo_bitRead($x,8) + 4 * SD_Rollo_bitRead($x,19) + 8 * SD_Rollo_bitRead($x,25) + 16 * SD_Rollo_bitRead($x,30);
		$bitVal = SD_Rollo_bitRead($x,31) ^ SD_Rollo_bitRead($x, 15) ^ SD_Rollo_bitRead($KeeLoq_NLF,$index) ^ $keyBitVal;
...
sub SD_Rollo_Parse($$) {
  ...
  # 15      15           1000 0000                   0000 0000           0000 0111
	...
	$modules{SD_Rollo}{defptr}{ioname} = $ioname;
  my $def = $modules{SD_Rollo}{defptr}{$devicedef." ".$typ};
  
	if(!$def) {
    Log3 $iohash, 2, "SD_Rollo_Parse Unknown device $typ with Code $devicedef detected, please define (rawdate=$rawData)";
    return "UNDEFINED SD_Rollo_".$devicedef." SD_Rollo ".$devicedef." ".$typ;
  }
	...
	Log3 $name, 4, "$ioname: SD_Rollo_Parse device $typ with rawData=$rawData, hlen=$hlen";
	...
	if (AttrVal($name, "verbose", "5") == 5) {
		if (defined $hash->{LASTInputDev}) {
			...
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - RAWMSG = ".$hash->{$RAWMSG_Internal};
		}
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - bitData = $bitData\n";
	}
	...
	if($typ eq "JaroLift") {	## for JaroLift Debug
		...
		$binsplit = SD_Rollo_binsplit_JaroLift($bitData);

		Log3 $name, 5, "$ioname: SD_Rollo_Parse - typ = $typ";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse                                 encoded     <- | ->     decrypts";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse             Grp 0-7 |digitS/N|      counter    | ch |          serial        | bt |Grp 8-15";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - bitData = $binsplit";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - bitData = |->     must be calculated!     <-| ".reverse (substr ($bitData , 32 , 4)) ." ". reverse (substr ($bitData , 36 , 24)) ." ".$button ." ". $bit64to71;
		...
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - group_value_text 8-15 (1)            = $group_value\n";
		...
		my $binsplit = SD_Rollo_binsplit_Roto($bitData);
		
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - typ = $typ";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse                                encoded     <- | ->     decrypts";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse                sync counter |discriminat.| bt |           serial           | bt |V|R|padding";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - bitData = $binsplit";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - bitData = |->     must be calculated!    <-| ". $serial ." ".$button ." ". $VLOW ." ". $RPT."\n";
	...
	if ($encrypted == 0) {
		...
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - input to decode                      = $Hopcode";
		my $Hopcode_decr = SD_Rollo_bin2dec($Hopcode);
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - HopCode - decrypts                   = $Hopcode_decr";
	
		my $keylow = $serial | 0x20000000;
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts (1)                         = $keylow";
	
		my $rx_device_key_lsb = SD_Rollo_decrypt($keylow, hex($MasterMSB), hex($MasterLSB), $name);
		$keylow =  $serial | 0x60000000;
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts (2)                         = $keylow";
	
		my $rx_device_key_msb = SD_Rollo_decrypt($keylow, hex($MasterMSB), hex($MasterLSB),$name);
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - rx_device_key_lsb                    = $rx_device_key_lsb";
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - rx_device_key_msb                    = $rx_device_key_msb";
	
		$Decoded = SD_Rollo_decrypt($Hopcode_decr, $rx_device_key_msb, $rx_device_key_lsb,$name);
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - Decoded (HopCode,MSB,LSB)            = $Decoded";
		$Decoded = SD_Rollo_dec2bin($Decoded);
		Log3 $name, 5, "$ioname: SD_Rollo_Parse - Decoded (bin)                        = $Decoded\n";
		...
		if ($typ eq "JaroLift") {
			...
			$counter_decr = SD_Rollo_bin2dec($counter);
			...
			Log3 $name, 5, "$ioname: SD_Rollo_Parse                                          Grp 0-7 |digitS/N|    counter";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - Decoded (bin split)                  = $Decoded_split\n";
			Log3 $name, 5, "######## DEBUG only with LSB & MSB Keys ########";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - channelpart1 (group 0-7)             = $bit0to7";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - group_value_text 0-7  (2)            = $group_value0_7";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - group_value_text 0-15 (3)            = $group_value";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - last_digits (bin)                    = ".substr($Decoded, 8, 8)." (only 4 bits ".substr($Decoded, 12, 4)." = decrypts ch reversed ".reverse (substr ($bitData , 32 , 4)).")";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - last_digits (channel from encoding)  = $bit8to15";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - countervalue (receive)               = $counter_decr\n";
		...
		if ($typ eq "Roto") {
			...
			$counter_decr = SD_Rollo_bin2dec($bit0to15_decr);

			Log3 $name, 5, "$ioname: SD_Rollo_Parse                                             sync counter |discriminat.| bt";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - Decoded (bin split)                  = $Decoded_split\n";
			Log3 $name, 5, "######## DEBUG only with LSB & MSB Keys ########";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - sync counter (bits)	          = $bit0to15_decr";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - sync counter (dez) 	          = $counter_decr";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - discrimination                       = $bit16to27";
			Log3 $name, 5, "$ioname: SD_Rollo_Parse - button (in encoded part)             = $bit28to31_decr = $buttonbits ???";
	...
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts button                      = $button";

	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts ch + serial                 = $serial (at each channel changes)" if ($typ eq "JaroLift");
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts ch + serial (bin)           = ".sprintf("%028b", $serial) if ($typ eq "JaroLift");
	
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts serial                      = $serialWithoutCh (for each channel)";
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts serial (bin)                = ".sprintf("%024b", $serialWithoutCh)." (for each channel)";
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts channel (from serial | bin) = $channel_bin" if (defined $channel_bin);
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts channel (from serial)       = $channel" if (defined $channel);
	
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts channelpart2 (group 8-15)   = $bit64to71" if (defined $bit64to71);
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts channel_control             = $group_value" if (defined $group_value);
	
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts Voltage LOW indicator       = $VLOW" if (defined $VLOW);
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - decrypts Repeat indicator            = $RPT" if (defined $RPT);
	
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - user_modus                           = $modus";
	Log3 $name, 5, "$ioname: SD_Rollo_Parse - user_info                            = $info";
...
sub SD_Rollo_Undef($$) {
	my ($hash, $name) = @_;
	delete($modules{SD_Rollo}{defptr}{$hash->{DEF}}) if(defined($hash->{DEF}) && defined($modules{SD_Rollo}{defptr}{$hash->{DEF}}));
	delete($modules{SD_Rollo}{defptr}{ioname}) if (exists $modules{SD_Rollo}{defptr}{ioname});
	return undef;
}

#####################################
sub SD_Rollo_binsplit_JaroLift($) {
	my $bits = shift;
	my $binsplit;
...
#####################################
sub SD_Rollo_summaryFn($$$$) {
	my ($FW_wname, $d, $room, $pageHash) = @_;										# pageHash is set for summaryFn.
	my $hash   = $defs{$d};
	my $name = $hash->{NAME};
	
	return SD_Rollo_attr2html($name, $hash);
}

#####################################
# Create HTML-Code
sub SD_Rollo_attr2html($@) {
  my ($name, $hash) = @_;
  ...
  if ($UI eq "Mehrzeilig") {
		if (not exists $attr{$name}{ChannelFixed}) {
			...
			foreach my $rownr (1..$Channels) {
				...
				$html.= SD_Rollo_attr2htmlButtons("$rownr", $name, $ShowIcons, $ShowShade, $ShowLearn);
				$html.= "</tr>";
			}
		} else {
				$html = "<div><table class=\"block wide\">";
				...
				$html.= SD_Rollo_attr2htmlButtons("$ChannelNum", $name, $ShowIcons, $ShowShade, $ShowLearn);
				$html.= "</tr>";
		}
		...
		foreach my $grp (@groups) {
			...
			$html.= "<tr><td>";
			$html.= $grpName."</td>";
			$html.= SD_Rollo_attr2htmlButtons($grpInfo[1], $name, $ShowIcons, 0, 0);
			$html.= "</tr>";
		}
  ...
  if ($UI eq "Einzeilig") {
		if (not exists $attr{$name}{ChannelFixed}) {
			...
			$html.= "</select></td>";
			$html.= SD_Rollo_attr2htmlButtons("OptionValue", $name, $ShowIcons, $ShowShade, $ShowLearn);
			$html.= "</table></div>";
		}
		...
		if (exists $attr{$name}{ChannelFixed}) {
			...
			$html.= SD_Rollo_attr2htmlButtons($ChannelFixed, $name, $ShowIcons, $ShowShade, $ShowLearn);
			$html.= "</tr></table></div>";
		}
...
<a name="SD_Rollo"></a>
<h3>SD_Rollo</h3>
<ul>The module SD_Rollo is a <br>

	<b>Define</b><br>
	<ul><code>define &lt;NAME&gt; SD_Rollo &lt;Device-ID&gt;</code><br><br>
	<u>examples:</u>
		<ul>
		define &lt;NAME&gt; SD_Rollo 12345678<br>
		</ul>	</ul><br><br>
...
<a name="SD_Rollo"></a>
<h3>SD_Rollo</h3>
<ul>Das SD_Rollo Modul simuliert eine Fernbedienung zur Steuerung anlernbarer Jarolift oder Roto TDEF-Funk-Motoren für Rollläden und Markisen. Dieses Modul wurde nach Vorlage des Bastelbudenbuben Projektes gefertigt. 
	...
	<b>Define</b><br>
	<ul><code>define &lt;NAME&gt; SD_Rollo &lt;Serial&gt; &lt;Typ&gt;</code><br><br>
	Beispiele: <ul><li>define SD_Rollo_Device1 SD_Rollo 12345678 JaroLift</li>
	<li>define SD_Rollo_Device2 SD_Rollo 123456 Roto</li></ul>
	</ul><br><br>
